...
DECLARE
  ...
  target_room_id uuid := '22222222-2222-4222-8222-222222222222';
...
BEGIN
  ...
  INSERT INTO public.rooms(id,hotel,organization_slug,is_checkout_room,pms_metadata,
    ...
    VALUES(target_room_id,'Hotel Memories Budapest','rdhotels',true,
  ...
  SELECT * INTO review FROM public.housekeeping_stay_extension_reviews
    WHERE room_id=target_room_id AND hotel_id='memories-budapest';
  ...
  UPDATE public.rooms SET pms_metadata=pms_metadata WHERE id=target_room_id;
  ...
  -- An explicit manager date update without PMS metadata must still work.
  UPDATE public.rooms SET last_towel_change=work_day WHERE id=target_room_id;
  SELECT * INTO room_row FROM public.rooms WHERE id=target_room_id;
