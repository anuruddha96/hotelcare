-- Fix towel and linen change requirements function
-- Towels should only be required exactly on the 3rd night
-- Linen should only be required exactly on the 5th night
-- Also fix for checkout rooms
  ...
  IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    SET 
      towel_change_required = CASE 
        ...
        WHEN guest_nights = 3 THEN true  -- Only on exactly the 3rd night
      ...
      linen_change_required = CASE 
        WHEN guest_checkout_date IS NOT NULL THEN true  -- Always for checkout rooms  
        WHEN guest_nights = 5 THEN true  -- Only on exactly the 5th night
