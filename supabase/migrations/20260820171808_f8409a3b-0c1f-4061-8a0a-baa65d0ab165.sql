CREATE INDEX IF NOT EXISTS rate_change_audit_marker_idx
  ON public.rate_change_audit (hotel_id, stay_date, performed_at DESC)
  WHERE payload ? 'room_type_name';
...
BEGIN
  ...
  -- One expensive pass, one response: the calendar used to page this in 1000-row
  -- chunks, re-running the whole scan for every page, which took long enough
  -- that the dots never appeared.
