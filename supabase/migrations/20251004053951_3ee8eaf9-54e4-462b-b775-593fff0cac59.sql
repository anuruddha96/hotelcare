-- Update get_assignable_staff_secure to filter by assigned hotel
...
BEGIN
  ...
  -- Return housekeepers filtered by hotel if user has one assigned
...
-- Update get_employees_by_hotel to respect assigned hotel for admins too
...
BEGIN
  ...
  -- If user has a specific hotel assigned (regardless of role), filter by that hotel
  IF current_user_hotel IS NOT NULL THEN
    RETURN QUERY
