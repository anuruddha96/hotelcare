-- Production migration revenue_latest_snapshot_index_probe, applied 2026-09-21.
-- A DISTINCT ON over historical snapshot rows scanned ~427k rows for Ottofiori
-- (10.59s). The existing (hotel_id,stay_date,captured_at DESC) index makes a
-- per-day LATERAL LIMIT 1 lookup ~43ms with the same 366 result rows.
-- Keep all historical snapshots; change only how the latest are selected.
CREATE OR REPLACE FUNCTION public.refresh_revenue_published_payload(_hotel_id text, _completed_at timestamptz DEFAULT now(), _actor_name text DEFAULT NULL::text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_org IS NULL THEN RAISE EXCEPTION 'Unknown hotel'; END IF;
  ...
  SELECT jsonb_build_object(
    'roomTypes', COALESCE((
      ...
      FROM (
        SELECT id, name, pms_room_id, num_rooms, is_reference, derivation_mode,
               ...
               name_translations
        FROM public.room_types WHERE hotel_id = _hotel_id
      ) r
    ), '[]'::jsonb),
    'nights', COALESCE((
    ...
    'soldOutPrices', COALESCE((
      ...
      FROM (
        ...
        WHERE hotel_id = _hotel_id AND released_at IS NULL
    ...
    'snapshots', COALESCE((
      ...
      FROM generate_series(v_from, v_to, interval '1 day') AS d(stay_date)
      CROSS JOIN LATERAL (
        SELECT stay_date, captured_date, captured_at, rooms_sold,
               rooms_available, occupancy_pct, revenue_eur, adr_eur, new_bookings
        FROM public.revenue_daily_snapshots
        WHERE hotel_id = _hotel_id AND stay_date = d.stay_date::date
    ...
    'settings', COALESCE((
      ...
      FROM (
        ...
        FROM public.hotel_revenue_settings WHERE hotel_id = _hotel_id
      ) s
    ), '{}'::jsonb)
  ) INTO v_payload;
