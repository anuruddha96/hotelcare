-- Make the Memories bed lifecycle safe across repeated checkout PMS refreshes.
-- A checkout retry must keep the previous-guest context even though the active
-- bed_configuration override was already cleared by the first checkout sync.
...
begin
  ...
  if is_checked_out then
    ...
    if old_previous_note is not null then
      new.notes := nullif(btrim(replace(coalesce(new.notes, ''), old_previous_note, '')), '');
    end if;

    previous_bed := coalesce(
      nullif(btrim(coalesce(old.bed_configuration, '')), ''),
      nullif(btrim(coalesce(old.pms_metadata #>> '{previousGuestBedSetup,value}', '')), '')
    );
...
-- Clean any duplicate context that may have been produced while upgrading the
-- previous trigger version. Keep exactly one generated line per room.
