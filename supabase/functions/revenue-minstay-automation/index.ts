...
function decideMinStay(f: DecisionFacts, maxNights: number): { target: number; reason: string } {
  ...
  if (monthOcc > 0 && monthOcc < 45) return { target: 1, reason: `soft_month_${Math.round(monthOcc)}pct_release` };
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [snapshotRes, minStayRes, pickupRes, eventRes, recentDecisionRes] = await Promise.all([
    ...
    for (const response of [snapshotRes, minStayRes, pickupRes, eventRes, recentDecisionRes]) {
    ...
    for (let d = today; d <= horizonDate; d = addDays(d, 1)) {
      ...
      if (changed && (!inCooldown || releaseOverride)) {
        proposals.push({ stayDate: d, current, target: decision.target, reason: decision.reason });
      }
    }
