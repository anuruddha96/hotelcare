...
/**
 * SLNT-only: reconcile PMS-owned free text without replacing a manager's room
 * instructions. Keep the two service flags in rooms.notes intact. The last
 * successfully synced PMS note is stored in pms_metadata so only text that
 * HotelCare itself wrote may be overwritten or cleared by a later sync.
 * A manager edit is never silently overwritten.
 */
export function reconcileSlntPrevioRoomNote(
  currentNotes: string | null | undefined,
  previousPmsNote: string | null | undefined,
  incomingPmsNote: string | null | undefined,
  ...
  // PMS-owned. Any other free text belongs to a human and must survive sync.
  const pmsOwnsText = !freeText || (!!previous && freeText === previous);
  if (!pmsOwnsText) {
    return { notes: current || null, changed: false, managerNotePreserved: true };
  }
  const next = [...flags, incoming].filter(Boolean).join(" ") || null;
  return {
    notes: next,
    changed: next !== (current || null),
    managerNotePreserved: false,
  };
}
