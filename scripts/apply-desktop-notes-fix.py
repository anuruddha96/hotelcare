...
path = Path('src/components/dashboard/HotelRoomOverview.tsx')
source = path.read_text()

# State: one debounce timer and a small save-state indicator for the desktop popover.
state_old = "  const [popoverNotes, setPopoverNotes] = useState<string>('');\n"
state_new = """  const [popoverNotes, setPopoverNotes] = useState<string>('');
...
if state_old not in source:
    raise SystemExit('Could not find popover notes state anchor')
source = source.replace(state_old, state_new, 1)

# Shared save helper keeps the existing room-service flags while replacing only the manager text.
helper_anchor = "  const [roomNotes, setRoomNotes] = useState('');\n"
helper = """
...
if helper_anchor not in source:
    raise SystemExit('Could not find room notes helper anchor')
source = source.replace(helper_anchor, helper_anchor + helper, 1)

# Reset the save indicator whenever a different room popover is opened.
hover_old = "      setPopoverNotes(flags.cleanNotes);\n"
hover_new = "      setPopoverNotes(flags.cleanNotes);\n      setPopoverNotesSaveState('idle');\n"
if hover_old not in source:
    raise SystemExit('Could not find hover notes anchor')
source = source.replace(hover_old, hover_new, 1)

# Replace the blur-only desktop editor with explicit Save + debounced autosave.
block_start = source.index("              {/* Notes - auto-save on blur */}")
block_end = source.index("              {/* Send Message to Housekeeper */}", block_start)
new_block = """              {/* Notes - explicit save + debounced auto-save while typing */}
              {isManagerOrAdmin && (
