...
export function CheckInDialog({ reservation, hotelId, open, onOpenChange, onSuccess }: CheckInDialogProps) {
  ...
  useEffect(() => {
    ...
    (supabase as any)
      .rpc('pms_available_rooms', {
        p_hotel_id: effectiveHotelId,
        p_check_in: reservation.check_in_date,
        p_check_out: reservation.check_out_date,
        p_exclude_reservation: reservation.id,
      })
      .then(({ data, error }: any) => {
        if (cancelled) return;
        if (error) {
          toast.error(error.message || t('pms.checkIn.failedCheckIn'));
          setRooms([]);
        } else {
          const available = Array.isArray(data) ? data : [];
          setRooms(available);
          if (!selectedRoom && reservation.room_id && available.some((r: any) => r.room_id === reservation.room_id)) {
  ...
  const handleCheckIn = async () => {
    ...
    setSubmitting(true);
    const { error } = await (supabase as any).rpc('pms_check_in', {
    ...
    toast.success(`${guestLabel} ${t('pms.checkIn.checkedIn')}`);
    onSuccess();
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md" data-training="reception-check-in-dialog">
        ...
        <div className="space-y-4">
          ...
          {(missingContact || missingDocument || balance > 0) && (
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription className="space-y-1 text-xs">
                {missingContact && <div>{t('pms.guests.noEmail')} / {t('pms.guests.noPhone')}</div>}
                {missingDocument && <div>{t('pms.guestDetail.idDocument')}: {t('pms.reservationDetail.notSpecified')}</div>}
                {balance > 0 && <div>{t('pms.reservationDetail.balance')}: {balance.toLocaleString()} {reservation.currency || 'HUF'}</div>}
              </AlertDescription>
            </Alert>
          )}
          ...
          <div>
            ...
            <Select value={selectedRoom} onValueChange={setSelectedRoom} disabled={loadingRooms}>
              <SelectTrigger data-training="reception-check-in-room">
                <SelectValue placeholder={loadingRooms ? t('pms.loading') : t('pms.checkIn.selectCleanRoom')} />
              </SelectTrigger>
              <SelectContent>
                {rooms.map((room) => (
        ...
        <DialogFooter>
          ...
          <Button data-training="reception-check-in-confirm" onClick={handleCheckIn} disabled={submitting || loadingRooms} className="gap-1">
