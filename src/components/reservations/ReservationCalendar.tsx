...
export function ReservationCalendar({ reservations, hotelId, days = 14 }: ReservationCalendarProps) {
  ...
  useEffect(() => {
    ...
    (supabase as any).rpc('pms_hotel_room_keys', { _hotel_id: hotelId }).then(async ({ data }: any) => {
      const keys = Array.isArray(data) && data.length ? data.map((v: any) => typeof v === 'string' ? v : (v.pms_hotel_room_keys || v.value)).filter(Boolean) : [hotelId];
      const { data: roomData } = await (supabase as any).from('rooms').select('id, room_number, room_type, status, is_checkout_room').in('hotel', keys).order('room_number');
      setRooms(roomData || []);
    });
  }, [hotelId]);
