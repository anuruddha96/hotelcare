...
export function ParkingHistory({ organizationSlug, hotelId, access, refreshVersion, onChanged }: Props) {
                ...
                <TableBody>
                  {tickets.map((ticket) => (
                    <TableRow key={ticket.id} className="cursor-pointer" onClick={() => setSelected(ticket)}>
                      ...
                      <TableCell><ParkingStatusBadge status={parkingDisplayStatus(ticket)} /></TableCell>
            ...
            <div className="space-y-2 md:hidden">
              {tickets.map((ticket) => (
                <button key={ticket.id} type="button" onClick={() => setSelected(ticket)} className="w-full rounded-lg border p-3 text-left transition-colors hover:bg-muted/50">
                  <div className="flex items-start justify-between gap-2"><p className="font-mono font-bold">{ticket.reference}</p><ParkingStatusBadge status={parkingDisplayStatus(ticket)} /></div>
