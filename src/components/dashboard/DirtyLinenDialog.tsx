...
const displayName = (item: Pick<LinenItem, 'name' | 'display_name'>, t: (key: string) => string) =>
  translateLinenItem(item.display_name || item.name.replace(/_/g, ' '), t);

/** Every hotel uses this same component. Writes stay on the existing counts
 * table and preserve its RLS, uniqueness, realtime subscriptions and audit. */
export function DirtyLinenDialog({ open, onOpenChange, roomId, roomNumber, assignmentId }: DirtyLinenDialogProps) {
    ...
    const instance = new DirtyLinenReliableQueue({
      ...
      save: async (itemId, count) => {
        if (count === 0) {
          ...
          if (error) throw error; // Deletion failure MUST NOT count as saved.
        } else {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      const { data, error } = await supabase.from('dirty_linen_items')
        .select('id, name, display_name, sort_order').eq('is_active', true).order('sort_order');
      if (error) toast.error('Failed to load linen items');
      else setLinenItems(data || []);
    };
    void fetchItems();
    void fetchMyRecords();
  }, [open, fetchMyRecords]);
    ...
    <DialogContent className="max-w-2xl w-[96vw] max-h-[92vh] h-[92vh] flex flex-col p-3 sm:p-6 gap-2 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto pr-1">
        {showMyRecords ? <div className="space-y-4">
          ...
          {!myRecords.length ? <p className="py-10 text-center text-muted-foreground">
            {t('dirtyLinen.noItemsCollected')}
          </p> : <div className="space-y-3">{myRecords.map((record, index) => <Card key={record.id} className="border-l-4 border-l-primary p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="space-y-1 min-w-0">
                ...
                <p className="text-sm font-medium break-words">
                  {displayName({ name: record.linen_item_name, display_name: record.display_name }, t)} × {record.count}
                </p>
                <p className="text-xs text-muted-foreground">{record.work_date}</p>
              </div>
              <AlertDialog>
                <AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="text-destructive shrink-0">
                ...
                <AlertDialogContent>
                  <AlertDialogHeader><AlertDialogTitle>{t('dirtyLinen.removeConfirmTitle')}</AlertDialogTitle>
                    <AlertDialogDescription>{t('dirtyLinen.removeConfirmDescription')
                      .replace('{item}', displayName({ name: record.linen_item_name, display_name: record.display_name }, t))
                  ...
                  <AlertDialogFooter><AlertDialogCancel>{t('common.cancel')}</AlertDialogCancel>
                    <AlertDialogAction onClick={() => { void deleteRecord(record); }} className="bg-destructive">
                      {t('dirtyLinen.remove')}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
          ...
          <div className="mb-2 flex items-center justify-between gap-2 rounded-md border bg-muted/40 px-3 py-2" aria-live="polite">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              {snapshot.status === 'saved' && <span className="flex items-center gap-1 text-xs text-green-700">
                <CheckCircle className="h-3 w-3" /> {t('dirtyLinen.saved')}
              </span>}
              {(snapshot.status === 'pending' || snapshot.status === 'saving') && <span className="text-xs text-muted-foreground">
                {t('dirtyLinen.saving')} ({snapshot.pending})
              </span>}
              {snapshot.status === 'error' && <span role="alert" className="flex items-center gap-1 text-xs text-destructive">
                <AlertTriangle className="h-3 w-3" /> Not synced ({snapshot.pending})
              </span>}
            </div>
          </div>
          {snapshot.error && <div role="alert" className="mb-2 rounded-md border border-destructive/40 p-2 text-xs">
            ...
            <Button size="sm" variant="outline" onClick={() => queue?.flushNow()} className="mt-2 gap-1">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-1 w-full">
                  <Button type="button" variant="outline" size="sm" className="h-9 w-9 p-0 shrink-0"
                    ...
                    onClick={() => adjust(item.id, -1)} aria-label={`Remove one ${displayName(item, t)}`}>
                  ...
                  <Input type="number" min="0" max="1000" inputMode="numeric"
                    ...
                    aria-label={`${displayName(item, t)} quantity`} />
                  <Button type="button" variant="outline" size="sm" className="h-9 w-9 p-0 shrink-0"
                    disabled={!loaded || (snapshot.counts[item.id] ?? 0) >= 1000}
                    onClick={() => adjust(item.id, 1)} aria-label={`Add one ${displayName(item, t)}`}>
