...
type UiCopy = {
  ...
  noOnDuty: string; noOnDutyDetail: string; ticketCreated: string;
  reportFailed: string; descriptionRequired: string; selectRoomRequired: string;
  photoAdded: string; photoCaptured: string;
};
...
const copyByLanguage: Record<string, UiCopy> = {
  en: {
    ...
    descriptionRequired: 'Please describe the problem', selectRoomRequired: 'Please select a room',
  ...
  hu: {
    ...
    descriptionRequired: 'Kérjük, írja le a problémát', selectRoomRequired: 'Kérjük, válasszon szobát',
  ...
  es: {
    ...
    ticketCreated: 'Ticket de mantenimiento creado', reportFailed: 'No se pudo informar el problema',
    descriptionRequired: 'Describa el problema', selectRoomRequired: 'Seleccione una habitación',
    photoAdded: 'foto(s) añadida(s)', photoCaptured: 'Foto capturada',
  },
  vi: {
    ...
    ticketCreated: 'Đã tạo phiếu bảo trì', reportFailed: 'Không thể báo sự cố bảo trì',
    descriptionRequired: 'Vui lòng mô tả sự cố', selectRoomRequired: 'Vui lòng chọn phòng',
    photoAdded: 'ảnh đã thêm', photoCaptured: 'Đã chụp ảnh',
  },
  mn: {
    ...
    ticketCreated: 'Засварын тасалбар үүслээ', reportFailed: 'Засварын асуудлыг мэдээлж чадсангүй',
    descriptionRequired: 'Асуудлыг тайлбарлана уу', selectRoomRequired: 'Өрөө сонгоно уу',
    photoAdded: 'зураг нэмэгдлээ', photoCaptured: 'Зураг авлаа',
  },
  az: {
    ...
    ticketCreated: 'Texniki xidmət tapşırığı yaradıldı', reportFailed: 'Texniki problemi bildirmək mümkün olmadı',
    descriptionRequired: 'Problemi təsvir edin', selectRoomRequired: 'Otaq seçin',
    photoAdded: 'şəkil əlavə edildi', photoCaptured: 'Şəkil çəkildi',
  },
  tl: {
    ...
    ticketCreated: 'Nagawa ang maintenance ticket', reportFailed: 'Hindi na-report ang maintenance issue',
    descriptionRequired: 'Pakilarawan ang problema', selectRoomRequired: 'Pumili ng kuwarto',
    photoAdded: 'larawan ang idinagdag', photoCaptured: 'Nakuha ang larawan',
  },
  uk: {
    ...
    assignedTo: 'Призначено', noOnDuty: 'Зараз немає технічного працівника, який увійшов у систему',
    noOnDutyDetail: 'Заявка залишиться видимою керівникам і буде автоматично призначена після входу технічного працівника.',
    ticketCreated: 'Заявку на ремонт створено', reportFailed: 'Не вдалося повідомити про проблему',
    descriptionRequired: 'Опишіть проблему', selectRoomRequired: 'Виберіть кімнату',
    photoAdded: 'фото додано', photoCaptured: 'Фото зроблено',
  },
  ru: {
    ...
    ticketCreated: 'Заявка на ремонт создана', reportFailed: 'Не удалось сообщить о проблеме',
    descriptionRequired: 'Опишите проблему', selectRoomRequired: 'Выберите комнату',
    photoAdded: 'фото добавлено', photoCaptured: 'Фото сделано',
  },
};

type SelectableRoom = { id: string; room_number: string; hotel: string; label?: string };
...
export function MaintenanceIssueDialog({ open, onOpenChange, roomNumber, roomId: initialRoomId, assignmentId, onIssueReported }: MaintenanceIssueDialogProps) {
  ...
  const [rooms, setRooms] = useState<SelectableRoom[]>([]);
  ...
  const filteredRooms = rooms.filter(room => !roomSearch.trim()
  ...
  useEffect(() => {
    if (!open) return;
    setRoomSearch('');
    if (!initialRoomId) void fetchRooms();
    else setSelectedRoomId(initialRoomId);
  }, [open, initialRoomId, profile?.assigned_hotel, profile?.organization_slug]);
  ...
  const fetchRooms = async () => {
    ...
    try {
      if (isGozsdu) {
        const mapped = await loadGozsduMappedOperatingRooms(profile?.organization_slug || '');
        setRooms(mapped);
        return;
      }
      ...
      if (profile?.assigned_hotel) {
        const { data: hotelConfig } = await supabase
          .from('hotel_configurations').select('hotel_name').eq('hotel_id', profile.assigned_hotel).maybeSingle();
  ...
  const handleSubmit = async () => {
    if (!user || !description.trim()) { toast.error(ui.descriptionRequired); return; }
    const roomToUse = initialRoomId || selectedRoomId;
    if (!roomToUse) { toast.error(ui.selectRoomRequired); return; }
    if (!initialRoomId && (roomsLoading || roomsError || !rooms.some(room => room.id === roomToUse))) {
    ...
    setIsUploading(true);
    try {
      // Recheck the live Team View map before uploading photos or filing a
      // manually selected Gozsdu maintenance ticket. Explicit room-card actions
      // remain possible for a known room already needing repair.
      if (isGozsdu && !initialRoomId) {
        const eligible = await loadGozsduMappedOperatingRooms(profile?.organization_slug || '');
        if (!eligible.some(room => room.id === roomToUse)) throw new Error('Room is not in the operating Team View map. Refresh the room list.');
      }
      const selectedRoom = rooms.find(room => room.id === roomToUse);
      ...
      const { data, error } = await (supabase as any).rpc('create_housekeeping_maintenance_ticket', {
        _room_id: roomToUse, _assignment_id: assignmentId || null,
      ...
      if (ticket?.assigned_to) {
        try {
          await supabase.functions.invoke('send-work-assignment-notification', {
            body: {
              staff_id: ticket.assigned_to, assignment_type: 'ticket',
              assignment_details: { id: ticket.id, title: description.trim().slice(0, 80), room_number: roomNum, priority },
              hotel_name: ticket.hotel,
            }
          });
        } catch (notificationError) { console.warn('Maintenance assignment email could not be sent:', notificationError); }
        ...
        <div className="space-y-4">
          {!initialRoomId && (
            <div className="space-y-2">
              <Label htmlFor="maintenance-room-search">{t('maintenance.selectRoom')} *</Label>
              {isGozsdu && <Input id="maintenance-room-search" value={roomSearch} onChange={event => setRoomSearch(event.target.value)} placeholder={language === 'hu' ? 'Szoba vagy épület keresése' : 'Search room or building'} />}
              <Select value={selectedRoomId || ''} disabled={roomsLoading || roomsError} onValueChange={setSelectedRoomId}>
                <SelectTrigger className="h-11"><SelectValue placeholder={t('maintenance.selectRoomPlaceholder')} /></SelectTrigger>
                <SelectContent>
                  {filteredRooms.map(room => <SelectItem key={room.id} value={room.id}>
                    {room.label || `${t('common.room')} ${room.room_number} · ${room.hotel}`}
                  </SelectItem>)}
                </SelectContent>
              </Select>
              {roomsLoading && <p className="text-xs text-muted-foreground">Loading mapped rooms…</p>}
              {roomsError && <Button size="sm" type="button" variant="outline" onClick={() => void fetchRooms()}>Retry room lookup</Button>}
              {!roomsError && !roomsLoading && isGozsdu && rooms.length === 0 && <p className="text-xs text-amber-700">No verified operating rooms in Team View.</p>}
            </div>
          )}
          <div className="space-y-2">
          ...
          {photos.length > 0 && <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {photos.map((photo, index) => <Card key={index} className="relative overflow-hidden">
                ...
                <Button type="button" size="icon" variant="destructive" className="absolute top-1 right-1 h-7 w-7" onClick={() => {
                  URL.revokeObjectURL(photo.dataUrl); setPhotos(prev => prev.filter((_, i) => i !== index));
                }}><X className="h-3.5 w-3.5" /></Button>
              </Card>)}
            </div>
          </div>}
          <div className="flex items-center gap-2 text-xs text-muted-foreground rounded-lg bg-muted/40 p-2.5">
            <UserCheck className="h-4 w-4 shrink-0" /><span>{ui.assignmentInfo}</span>
          </div>
          <div className="grid grid-cols-2 gap-2 pt-1">
            <Button type="button" onClick={handleClose} variant="outline" className="h-11" disabled={isUploading}>{t('common.cancel')}</Button>
            <Button type="button" onClick={handleSubmit}
              disabled={isUploading || !description.trim() || (!initialRoomId && (roomsLoading || roomsError || !selectedRoomId))} className="h-11">
