...
type MaintenanceStaff = {
  id: string; full_name: string; role: string; assigned_hotel: string | null;
  is_signed_in: boolean; checked_in_at: string | null;
};
...
const copy = {
  en: {
    create: 'Create ticket', intro: 'Report one issue for the selected property. Housekeeping and maintenance share the same ticket.',
    ...
    cancel: 'Cancel', submit: 'Create ticket', creating: 'Creating…', selectHotel: 'Select hotel', optional: 'Optional',
    attachments: 'Attachments', success: 'Ticket created', assigned: 'Assigned to', queued: 'Queued for maintenance',
    error: 'Could not create ticket', required: 'Complete the required fields', invalidRoom: 'Select an eligible room from this property.',
  ...
  hu: {
    create: 'Jegy létrehozása', intro: 'Jelentsen egy hibát a kiválasztott hotelben. A takarítás és a karbantartás ugyanazt a jegyet használja.',
    ...
    cancel: 'Mégse', submit: 'Jegy létrehozása', creating: 'Létrehozás…', selectHotel: 'Hotel kiválasztása', optional: 'Opcionális',
    attachments: 'Mellékletek', success: 'Jegy létrehozva', assigned: 'Hozzárendelve', queued: 'Karbantartási sorban',
...
const initialForm = () => ({ title: '', description: '', location: '', roomId: null as string | null,
  priority: 'medium' as 'low' | 'medium' | 'high' | 'urgent', department: 'maintenance', hotel: '' });
...
export function CreateTicketDialog({ open, onOpenChange, onTicketCreated }: CreateTicketDialogProps) {
  ...
  useEffect(() => {
    ...
    void loadMaintenanceRoomOptions(hotelKeys, profile.organization_slug).then(options => {
      if (!cancelled) setRooms(options);
    }).catch(error => {
      if (cancelled) return;
      console.error('Maintenance room lookup failed:', error);
      setRoomsError(true);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      // Re-read the operational inventory to catch PMS changes or another tab
      // withdrawing this room. The database insert guard performs final validation.
      ...
      const { data, error } = await (supabase as any).from('tickets').insert({
        title: formData.title.trim(), description,
        room_number: room?.roomNumber || 'N/A', source_room_id: room?.id || null,
      ...
      if (attachmentRef.current?.hasAttachments()) {
        try {
          ...
          if (uploadedPaths.length) {
            const { error: attachError } = await supabase.from('tickets')
              .update({ attachment_urls: uploadedPaths }).eq('id', data.id)
              .eq('organization_slug', profile.organization_slug).eq('hotel', selectedHotel.hotel_name);
      ...
      if (savedTicket) {
        ...
        toast({ title: `${c.success} · ${savedTicket.ticket_number}` });
      } else {
      ...
      <DialogContent className="w-[calc(100vw-1rem)] sm:max-w-2xl max-h-[94vh] overflow-y-auto p-4 sm:p-6">
        ...
        {!canCreateTickets ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formData.hotel} onValueChange={hotel => {
                  ...
                  setFormData(previous => ({ ...previous, hotel, roomId: null }));
                  setSelectedMaintenancePerson('auto');
                }}>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2"><Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={loading} className="h-11">{c.cancel}</Button><Button type="submit" disabled={loading || roomsLoading || roomsError || !selectedHotel || !canCreateTickets} className="h-11">{loading ? c.creating : <><CheckCircle2 className="h-4 w-4 mr-2" />{c.submit}</>}</Button></div>
          </form>
        )}
      </DialogContent>
    </Dialog>
