...
export function DirtyLinenItemsManagement() {
  ...
  const [formData, setFormData] = useState({ name: '', display_name: '', is_active: true, sort_order: 0 });
  ...
  useEffect(() => { void fetchLinenItems(); }, [profile?.assigned_hotel]);

  const handleAdd = () => {
    if (gozsdu) return;
    const maxSortOrder = Math.max(...linenItems.map(item => item.sort_order), 0);
    setEditingItem(null);
    setFormData({ name: '', display_name: '', is_active: true, sort_order: maxSortOrder + 1 });
  ...
  const handleSave = async () => {
    if (gozsdu) return;
    if (!formData.name || !formData.display_name) { toast.error('Please fill in all required fields'); return; }
    try {
      const itemData = { name: formData.name.toLowerCase().replace(/\s+/g, '_'), display_name: formData.display_name,
        is_active: formData.is_active, sort_order: formData.sort_order };
      if (editingItem) {
        const { error } = await supabase.from('dirty_linen_items').update(itemData).eq('id', editingItem.id);
        if (error) throw error;
        toast.success('Linen item updated successfully');
        setIsEditDialogOpen(false);
      } else {
        const { error } = await supabase.from('dirty_linen_items').insert([itemData]);
        if (error) throw error;
  ...
  const ItemForm = () => <div className="space-y-4">
    <div className="space-y-2"><Label htmlFor="display_name">Display Name *</Label><Input id="display_name"
      value={formData.display_name} onChange={e => setFormData({ ...formData, display_name: e.target.value })} placeholder="e.g., Pillow Cases" /></div>
    <div className="space-y-2"><Label htmlFor="name">Internal Name *</Label><Input id="name" value={formData.name}
      onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g., pillow_cases" /></div>
    <div className="space-y-2"><Label htmlFor="sort_order">Sort Order</Label><Input id="sort_order" type="number" value={formData.sort_order}
      onChange={e => setFormData({ ...formData, sort_order: parseInt(e.target.value) || 0 })} /></div>
    ...
    <div className="flex gap-2 pt-4"><Button variant="outline" onClick={() => { setIsAddDialogOpen(false); setIsEditDialogOpen(false); }} className="flex-1">Cancel</Button>
      <Button onClick={handleSave} className="flex-1">{editingItem ? 'Update' : 'Add'} Item</Button></div>
  </div>;
  ...
  return <div className="space-y-6">
    <div className="flex justify-between items-center gap-2">
      ...
      {!gozsdu && <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        ...
        <DialogContent><DialogHeader><DialogTitle>Add New Linen Item</DialogTitle></DialogHeader><ItemForm /></DialogContent>
      </Dialog>}
    </div>
    {gozsdu && <p role="status" className="rounded-md border p-3 text-sm text-muted-foreground">
      Gozsdu Court Budapest: fixed 15-item paper-sheet catalogue. Order and categories are shared by housekeepers, Laundryners and managers; they cannot be altered here. Other hotels are unaffected.
    </p>}
    <Card><CardHeader><CardTitle>Linen Items</CardTitle></CardHeader><CardContent>
      {loading ? <div className="flex justify-center p-8"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary" /></div>
        : <div className="space-y-2">{linenItems.map(item => <div key={item.id}
          ...
          {!gozsdu && <div className="flex items-center gap-2"><Switch checked={item.is_active} onCheckedChange={() => { void toggleActive(item); }} />
            <Dialog open={isEditDialogOpen && editingItem?.id === item.id} onOpenChange={setIsEditDialogOpen}>
              ...
              <DialogContent><DialogHeader><DialogTitle>Edit Linen Item</DialogTitle></DialogHeader><ItemForm /></DialogContent>
            </Dialog>
            <AlertDialog><AlertDialogTrigger asChild><Button variant="outline" size="sm" className="text-destructive"><Trash2 className="h-3 w-3" /></Button></AlertDialogTrigger>
              <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Linen Item</AlertDialogTitle>
                <AlertDialogDescription>Are you sure you want to delete "{item.display_name}"? This will also delete all associated count records.</AlertDialogDescription>
              </AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={() => { void handleDelete(item); }}>Delete</AlertDialogAction></AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog></div>}
        </div>)}</div>}
    </CardContent></Card>
