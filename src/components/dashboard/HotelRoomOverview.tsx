      ...
      >
        ...
        >
          ...
          {(() => {
            ...
            // Never show C/O+1 on a room that is already a checkout today
            // (manual C/O, checkout assignment, or checked-out flag). The
            // guest is leaving today, so a "departs tomorrow" hint is noise.
            if (room.is_checkout_room) return null;
            if (meta.checkedOutToday === true) return null;
            const assignment = assignmentMap.get(room.id);
            if (assignment?.assignment_type === 'checkout_cleaning') return null;
