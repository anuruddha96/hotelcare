...
export default function ReceptionBreakfastUpload() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
        <Button variant="ghost" size="sm" className="gap-1" onClick={() => navigate(`${orgPath}/reception`)}>
          <ArrowLeft className="h-4 w-4" /> Reception
        </Button>
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Upload tonight's Daily Overview so the breakfast team can serve guests correctly tomorrow morning.
          </p>
        </div>
        ...
        <Card className="border-2 border-primary/20 shadow-sm">
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <Label className="text-xs" htmlFor="stay-date">Breakfast date</Label>
                <Input
                  id="stay-date"
                  type="date"
                  value={date}
            ...
            <Button onClick={upload} disabled={busy || !file || !hotelId} size="lg" className="w-full">
              {busy ? (
                <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Uploading…</>
              ) : (
                <><Upload className="h-4 w-4 mr-2" />Upload for {date}</>
              )}
            </Button>
            ...
            {last && (
              <div className="flex items-start gap-2 rounded-md border border-green-200 bg-green-50 dark:bg-green-950/30 dark:border-green-900 p-3 text-sm">
                ...
                <div className="space-y-1">
                  <p><span className="font-medium">{last.rows} rooms</span> uploaded from <span className="font-mono text-xs">{last.fileName}</span></p>
                  {last.dates.length > 0 && <p className="text-xs text-muted-foreground">Dates in file: {last.dates.join(", ")}</p>}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-base"><Clock className="h-4 w-4" />Recent uploads</CardTitle>
          </CardHeader>
          <CardContent>
            {recentLoading ? (
                ...
                {recent.map((r) => (
                  <li key={r.stay_date} className="flex items-center justify-between py-2">
                    <div className="flex items-center gap-3">
                      ...
                      {r.stay_date === tomorrowISO() && <Badge variant="secondary" className="text-[10px]">Tomorrow</Badge>}
                      {r.stay_date === todayISO && <Badge variant="secondary" className="text-[10px]">Today</Badge>}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {r.rows} rooms{r.last_uploaded_at && ` • ${new Date(r.last_uploaded_at).toLocaleString()}`}
                    </div>
                  </li>
                ))}
              </ul>
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">Browse (read-only)</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <Button variant="outline" className="justify-start" onClick={() => navigate(`${orgPath}?tab=tickets`)}><Ticket className="h-4 w-4 mr-2" /> Maintenance</Button>
            <Button variant="outline" className="justify-start" onClick={() => navigate(`${orgPath}?tab=rooms`)}><Home className="h-4 w-4 mr-2" /> Rooms</Button>
            <Button variant="outline" className="justify-start" onClick={() => navigate(`${orgPath}?tab=housekeeping`)}><Users className="h-4 w-4 mr-2" /> Housekeeping</Button>
            <Button variant="outline" className="justify-start" onClick={() => navigate(`${orgPath}?tab=attendance`)}><ClipboardList className="h-4 w-4 mr-2" /> Attendance</Button>
          </CardContent>
        </Card>
      </main>
    </div>
